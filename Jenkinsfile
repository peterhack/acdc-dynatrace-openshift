node {
    ...
    stage('DeployStaging') {
        ...
        app.run("--name SampleNodeJsStaging -p 8080:8080 " +
    ...
    stage('Testing') {
        ...
        dir ('sample-nodejs-service-tests') {
            // start load test and run for 120 seconds - simulating traffic for Staging enviornment on port 8080
            sh "rm -f stagingloadtest.log stagingloadtestcontrol.txt"
            sh "./loadtest.sh 8080 stagingloadtest.log stagingloadtestcontrol.txt 120 Staging"
    ...
    stage('DeployProduction') {
        ...
        def app = docker.image("sample-nodejs-service:${BUILD_NUMBER}")
        app.run("--name SampleNodeJsProduction -p 8090:8090 "+
    ...
    stage('WarmUpProduction') {
        ...
        // lets run some test scripts
        dir ('sample-nodejs-service-tests') {
            // start load test and run for 120 seconds - simulating traffic for Production enviornment on port 8090
            sh "rm -f productionloadtest.log productionloadtestcontrol.txt"
            sh "./loadtest.sh 8090 productionloadtest.log productionloadtestcontrol.txt 60 Production"
